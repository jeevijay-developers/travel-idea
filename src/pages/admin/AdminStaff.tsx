...
import { Plus, Trash2, Users, Shield, FileText, Plane } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { z } from "zod";

type AppRole = "admin" | "blog_editor" | "visa_manager" | "user";
...
const newStaffSchema = z.object({
  ...
  role: z.enum(["admin", "blog_editor", "visa_manager"]),
});
...
export default function AdminStaff() {
  ...
  const createStaffMutation = useMutation({
    mutationFn: async ({ email, password, role }: { email: string; password: string; role: AppRole }) => {
      // Use the edge function to create staff
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "The new staff member can now log in.",
      });
    },
    onError: (error: Error) => {
      toast({
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="staff@travelidea.in"
                />
              ...
              <div className="space-y-2">
                ...
                <Select value={role} onValueChange={(value) => setRole(value as AppRole)}>
                  ...
                  <SelectContent>
                    {(["admin", "blog_editor", "visa_manager"] as AppRole[]).map((r) => {
